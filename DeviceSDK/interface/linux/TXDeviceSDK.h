...
typedef struct tag_tx_binder_info
{
    ...
    unsigned long long uin;    //绑定者uin, 出于保护用户隐私的考虑，这个值总是0
...
/**
* 接口说明: 配网成功后通知手Q端，配网成功
* 参数说明: 
* ip   - 手Q端的ip，ip和port都可以从TXVoiceLink.h中FUNC_NOTIFY中获得
* port - 手Q端的端口
*/
 ...
 * 回调函数参数说明：
 *          log_func   回调函数
 *          log_write  是否写log到文件，0代表不写，1代表写；SDK可以将该log文件上传到小微后台，方便问题追踪, 所以建议填1。
 *          log_debug_print  是否允许SDK向写log的回调函数传递 冗余级别的log ， 0代表不传，1代表传。
 */
